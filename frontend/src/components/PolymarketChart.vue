  ...
  <div class="pm-panel">
    ...
    <div class="pm-header">
      <div class="pm-header-left">
        ...
        <span class="pm-header-title">{{ $tr('Prediction Markets', '预测市场', 'Vorhersagemärkte') }}</span>
        <span v-if="live" class="pm-live-dot"><span class="pm-live-pulse"></span>{{ $tr('LIVE', '实时', 'LIVE') }}</span>
      </div>
      <div v-if="selected" class="pm-header-actions">
        <button
          class="pm-export-btn"
          :disabled="exporting || !copySupported"
          :title="copySupported ? $tr('Copy chart as PNG (with MiroShark watermark)', '复制图表为 PNG(含 MiroShark 水印)', 'Diagramm als PNG kopieren (mit MiroShark-Wasserzeichen)') : $tr('Image copy not supported in this browser', '此浏览器不支持图像复制', 'Bild-Kopieren wird in diesem Browser nicht unterstützt')"
          @click="copyChart"
        >
      ...
      <div class="pm-body">
        ...
        <aside class="pm-market-list">
          <div v-if="marketsLoading && !markets.length" class="pm-empty">{{ $tr('Loading markets...', '加载市场中...', 'Märkte werden geladen...') }}</div>
          <div v-else-if="marketsError" class="pm-empty pm-error">{{ marketsError }}</div>
          <div v-else-if="!markets.length" class="pm-empty">
            <div class="pm-empty-title">{{ $tr('No markets yet', '暂无市场', 'Noch keine Märkte') }}</div>
            <div class="pm-empty-hint">{{ $tr('Markets appear as agents create them during the simulation.', '智能体在模拟过程中创建市场后,将显示在此处。', 'Märkte erscheinen, sobald Agenten sie während der Simulation erstellen.') }}</div>
          </div>
          <button
            v-for="m in markets"
            :key="m.market_id"
          ...
          >
            <div class="pm-market-q">{{ m.question || `${$tr('Market', '市场', 'Markt')} #${m.market_id}` }}</div>
            <div class="pm-market-meta">
              <span class="pm-market-price" :class="priceClass(m.price_yes)">
                {{ formatPct(m.price_yes) }}
              </span>
              <span class="pm-market-trades">{{ m.trade_count }} {{ $tr('trades', '笔交易', 'Trades') }}</span>
              <span v-if="m.resolved" class="pm-market-resolved">{{ m.winning_outcome || $tr('RESOLVED', '已结算', 'ABGERECHNET') }}</span>
            </div>
          </button>
        </aside>
        ...
        <section class="pm-chart-section">
          <div v-if="!selected" class="pm-placeholder">
            ...
            <div class="pm-placeholder-text">{{ $tr('Select a market to view its price history', '选择一个市场以查看其价格历史', 'Markt auswählen, um den Preisverlauf anzuzeigen') }}</div>
          </div>
          <template v-else>
            <!-- Question + price header -->
            <div class="pm-chart-header">
              <div class="pm-chart-q">{{ selected.market.question || `${$tr('Market', '市场', 'Markt')} #${selected.market.market_id}` }}</div>
              <div class="pm-chart-price-row">
                <div class="pm-chart-price" :class="priceClass(latestPrice)">
                  {{ formatPct(latestPrice) }}
                  <span class="pm-chart-outcome-label">{{ $tr('chance', '概率', 'Chance') }} {{ selected.market.outcome_a || 'YES' }}</span>
                </div>
              ...
              <div class="pm-chart-stats">
                <span class="pm-stat"><span class="pm-stat-k">{{ $tr('TRADES', '交易', 'TRADES') }}</span><span class="pm-stat-v">{{ selected.points.length - 1 }}</span></span>
                <span class="pm-stat"><span class="pm-stat-k">{{ $tr('VOLUME', '成交量', 'VOLUMEN') }}</span><span class="pm-stat-v">{{ tradeVolume.toFixed(1) }}</span></span>
                <span class="pm-stat"><span class="pm-stat-k">{{ $tr('OUTCOMES', '结果', 'ERGEBNISSE') }}</span><span class="pm-stat-v">{{ selected.market.outcome_a }}/{{ selected.market.outcome_b }}</span></span>
                <span v-if="selected.market.resolved" class="pm-stat pm-stat-resolved">{{ $tr('RESOLVED:', '已结算:', 'ABGERECHNET:') }} {{ selected.market.winning_outcome }}</span>
              </div>
            </div>
            ...
            <div class="pm-chart-wrap">
              ...
              <div v-if="hoverPoint !== null && hoverTooltipStyle" class="pm-tooltip" :style="hoverTooltipStyle">
                ...
                <div v-else class="pm-tooltip-trade">{{ $tr('Origin (no trades yet)', '初始(尚无交易)', 'Start (noch keine Trades)') }}</div>
...
async function fetchMarkets({ autoSelect = false } = {}) {
  ...
  try {
    ...
    marketsError.value = err?.response?.data?.error || err?.message || tr('Failed to load markets', '加载市场失败', 'Märkte konnten nicht geladen werden')
  } finally {
...
async function fetchSelected() {
  ...
  try {
    ...
    selectedError.value = err?.response?.data?.error || err?.message || tr('Failed to load prices', '加载价格失败', 'Preise konnten nicht geladen werden')
  }
}
