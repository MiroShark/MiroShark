<template>
  <transition name="ss-fade">
    <div v-if="shouldShow" class="ss-wrap">
      <div class="ss-head">
        <span class="ss-label">
          <span class="ss-dot">◈</span> {{ $tr('Smart Setup', '智能设置', 'Intelligente Einrichtung') }}
        ...
        <button
          ...
          class="ss-close"
          type="button"
          :title="$tr('Dismiss suggestions', '关闭建议', 'Vorschläge schließen')"
          @click="dismiss"
        >×</button>
      </div>
      ...
      <div v-if="loading" class="ss-loading">
        ...
        {{ $tr('Drafting three scenarios from your document…', '正在根据你的文档起草三个情景…', 'Drei Szenarien aus Ihrem Dokument werden erstellt…') }}
      </div>
      ...
      <div v-else-if="suggestions.length > 0" class="ss-cards">
        ...
        >
          <div class="ss-card-head">
            <span class="ss-badge" :class="badgeClass(s.label)">{{ s.label === 'Bull' ? $tr('Bull', '看涨', 'Optimistisch') : s.label === 'Bear' ? $tr('Bear', '看跌', 'Pessimistisch') : s.label === 'Neutral' ? $tr('Neutral', '中立', 'Neutral') : s.label }}</span>
            <span class="ss-range">{{ $tr('Initial YES', '初始 YES', 'Initialer JA-Wert') }} {{ s.expected_yes_range[0] }}–{{ s.expected_yes_range[1] }}%</span>
          </div>
          ...
          >{{ $tr('Use this →', '使用 →', 'Verwenden →') }}</button>
        </div>
      </div>
...
const statusLine = computed(() => {
  if (loading.value) return tr('// generating…', '// 生成中…', '// wird generiert…')
  if (suggestions.value.length > 0) return tr('// pick one or refine your own', '// 选择一个或自行完善', '// eines auswählen oder eigenes verfeinern')
  return ''
})
