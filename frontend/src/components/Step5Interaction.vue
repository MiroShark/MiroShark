            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ $tr('Generating', '正在生成', 'Generiere') }} {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-text">
            <span class="action-bar-title">{{ $tr('Interactive Tools', '交互工具', 'Interaktive Tools') }}</span>
            <span class="action-bar-subtitle mono">{{ profiles.length }} {{ $tr('personas available', '个可用人设', 'Agenten verfügbar') }}</span>
          </div>
        </div>
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>{{ $tr('Chat with Report Agent', '与报告智能体对话', 'Mit Report Agent chatten') }}</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : $tr('Persona Chat', '人设对话', 'Mit beliebigem Individuum chatten') }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">{{ $tr('Select Chat Target', '选择对话对象', 'Gesprächsziel auswählen') }}</div>
                <div 
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || $tr('Unknown Profession', '未知职业', 'Unbekannter Beruf') }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <span>{{ $tr('Group Questions', '群组提问', 'Umfrage an die Welt senden') }}</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
            ...
            <div v-if="showToolsDetail" class="tools-card-body">
              <div class="tools-grid">
                ...
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $tr('PanoramaSearch Panoramic Tracking', 'PanoramaSearch 全景追踪', 'PanoramaSearch Panorama-Tracking') }}</div>
                    <div class="tool-desc">{{ $tr('Graph-based breadth traversal algorithm that reconstructs event propagation paths and captures the full topology of information flow', '基于图的广度遍历算法,重建事件传播路径并捕捉信息流的完整拓扑结构', 'Graphbasierter Breitensuchalgorithmus, der Ereignisausbreitungspfade rekonstruiert und die vollständige Topologie des Informationsflusses erfasst') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $tr('QuickSearch Quick Retrieval', 'QuickSearch 快速检索', 'QuickSearch Schnellabfrage') }}</div>
                    <div class="tool-desc">{{ $tr('GraphRAG-based instant query interface with optimized indexing efficiency for quickly extracting specific node attributes and discrete facts', '基于 GraphRAG 的即时查询接口,优化索引效率,快速提取特定节点属性和离散事实', 'GraphRAG-basierte Sofortabfrageschnittstelle mit optimierter Indizierungseffizienz zur schnellen Extraktion spezifischer Knotenattribute und diskreter Fakten') }}</div>
                  </div>
                </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info clickable" @click="openProfilePopup(selectedAgent)">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || $tr('Unknown Profession', '未知职业', 'Unbekannter Beruf') }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">{{ $tr('Bio', '简介', 'Biografie') }}</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? $tr('Chat with Report Agent to explore report details', '与报告智能体对话,探索报告详情', 'Mit Report Agent chatten, um Berichtsdetails zu erkunden') : $tr('Chat with simulated individuals to understand their perspectives', '与模拟个体对话,了解他们的观点', 'Mit simulierten Individuen chatten, um ihre Perspektiven zu verstehen') }}
              </p>
            </div>
            <div 
              ...
              <div class="message-content">
                <div class="message-header">
                  <span class="sender-name">
                    {{ msg.role === 'user' ? $tr('You', '你', 'Sie') : (chatTarget === 'report_agent' ? $tr('Report Agent', '报告智能体', 'Report Agent') : (selectedAgent?.username || $tr('Agent', '智能体', 'Agent'))) }}
                  </span>
          ...
          <div class="chat-input-area">
            <textarea 
              v-model="chatInput"
              class="chat-input"
              :placeholder="$tr('Enter your question...', '请输入你的问题...', 'Frage eingeben...')"
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header section-header-toggle" @click="showPersonaSection = !showPersonaSection">
                <div class="section-header-left">
                  ...
                  <span class="section-title">{{ $tr('Select Persona', '选择人设', 'Persona auswählen') }}</span>
                </div>
                <span class="selection-count">{{ $tr('Selected', '已选', 'Ausgewählt') }} {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div v-show="showPersonaSection" class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || $tr('Unknown Profession', '未知职业', 'Unbekannter Beruf') }}</span>
                  </div>
              ...
              <div v-show="showPersonaSection" class="selection-actions">
                <button class="action-link" @click="selectAllAgents">{{ $tr('Select All', '全选', 'Alle auswählen') }}</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">{{ $tr('Clear', '清空', 'Auswahl aufheben') }}</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">{{ $tr('Persona Question', '人设问题', 'Persona-Frage') }}</span>
              </div>
              <textarea
                v-model="surveyQuestion"
                class="survey-input"
                :placeholder="$tr('Enter the question you want to ask all selected targets...', '输入你想向所有选中对象提出的问题...', 'Frage eingeben, die Sie an alle ausgewählten Ziele stellen möchten...')"
                rows="3"
              ></textarea>
            </div>
            ...
            >
              ...
              <span v-else>{{ $tr('Send Question', '发送问题', 'Umfrage senden') }}</span>
            </button>
          </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">{{ $tr('Results', '结果', 'Ergebnisse') }}</span>
              <span class="results-count">{{ surveyResults.length }} {{ $tr('responses', '条回复', 'Antworten') }}</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || $tr('Unknown Profession', '未知职业', 'Unbekannter Beruf') }}</span>
                  </div>
                </div>
        ...
        <div class="profile-popup">
          ...
          <div class="profile-popup-details">
            <div class="profile-popup-row" v-if="profilePopupAgent.bio">
              <span class="popup-label">{{ $tr('Bio', '简介', 'Biografie') }}</span>
              <span class="popup-value">{{ profilePopupAgent.bio }}</span>
            </div>
            <div class="profile-popup-stats">
              <span v-if="profilePopupAgent.age" class="popup-stat">{{ profilePopupAgent.age }}{{ $tr('y', '岁', 'J') }}</span>
            ...
            <div class="profile-popup-row" v-if="profilePopupAgent.persona">
              <span class="popup-label" @click="showFullPersona = !showFullPersona" style="cursor:pointer">{{ $tr('Persona', '人设', 'Persona') }} {{ showFullPersona ? '▼' : '▶' }}</span>
          ...
          <div class="profile-popup-activity">
            <div class="popup-section-title">{{ $tr('Simulation Activity', '模拟活动', 'Simulationsaktivität') }} ({{ profilePopupActions.length }})</div>
            <div v-if="profilePopupLoading" class="popup-loading">{{ $tr('Loading...', '加载中...', 'Laden...') }}</div>
            <div v-else-if="profilePopupActions.length === 0" class="popup-empty">{{ $tr('No activity recorded', '没有活动记录', 'Keine Aktivität aufgezeichnet') }}</div>
            <div v-else class="popup-actions-list">
              ...
              >
                ...
                <div v-if="expandedActions.has(i)" class="popup-action-full">
                  ...
                  <p v-if="action.action_args?.quote_content"><strong>{{ $tr('Quote:', '引用:', 'Zitat:') }}</strong> {{ action.action_args.quote_content }}</p>
                  <p v-if="action.action_args?.post_content"><strong>{{ $tr('Original:', '原文:', 'Original:') }}</strong> {{ action.action_args.post_content }}</p>
                  <p v-if="action.action_args?.original_content"><strong>{{ $tr('Ref:', '引用:', 'Referenz:') }}</strong> {{ action.action_args.original_content }}</p>
                  <p v-if="action.action_args?.target_user_name"><strong>{{ $tr('Target:', '对象:', 'Ziel:') }}</strong> @{{ action.action_args.target_user_name }}</p>
                  <p v-if="action.action_args?.query"><strong>{{ $tr('Query:', '查询:', 'Abfrage:') }}</strong> {{ action.action_args.query }}</p>
                </div>
              </div>
            </div>
          </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(`${tr('Select chat target', '选择对话对象', 'Gesprächsziel auswählen')}: ${agent.username}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(`${tr('Send failed', '发送失败', 'Senden fehlgeschlagen')}: ${err.message}`)
    chatHistory.value.push({
      role: 'assistant',
      content: `${tr('Sorry, an error occurred', '抱歉,发生了错误', 'Es ist ein Fehler aufgetreten')}: ${err.message}`,
      timestamp: new Date().toISOString()
    })
  } finally {
...
const sendToReportAgent = async (message) => {
  addLog(`${tr('Sending to Report Agent', '发送至报告智能体', 'Sende an Report Agent')}: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || tr('No response', '无响应', 'Keine Antwort'),
      timestamp: new Date().toISOString()
    })
    addLog(tr('Report Agent has replied', '报告智能体已回复', 'Report Agent hat geantwortet'))
  } else {
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error(tr('Please select a simulated individual first', '请先选择一个模拟个体', 'Bitte wählen Sie zuerst ein simuliertes Individuum aus'))
  }
  
  addLog(`${tr('Sending to', '发送至', 'Sende an')} ${selectedAgent.value.username}: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} ${tr('has replied', '已回复', 'hat geantwortet')}`)
    } else {
      throw new Error(tr('No response data', '没有响应数据', 'Keine Antwortdaten'))
    }
  } else {
    throw new Error(res.error || tr('Request failed', '请求失败', 'Anfrage fehlgeschlagen'))
  }
...
const submitSurvey = async () => {
  ...
  addLog(`${tr('Sending survey to', '正在向', 'Sende Umfrage an')} ${selectedAgents.value.size} ${tr('targets...', '个对象发送问卷...', 'Ziele...')}`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        // Prefer reddit platform reply, then twitter
        let responseContent = tr('No response', '无响应', 'Keine Antwort')
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || tr('No response', '无响应', 'Keine Antwort')
          }
        } else if (Array.isArray(resultsDict)) {
          // Compatible with array format
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || tr('No response', '无响应', 'Keine Antwort')
          }
        }
      ...
      addLog(`${tr('Received', '已收到', 'Erhalten')} ${surveyResults.value.length} ${tr('replies', '条回复', 'Antworten')}`)
    } else {
      throw new Error(res.error || tr('Request failed', '请求失败', 'Anfrage fehlgeschlagen'))
    }
  } catch (err) {
    addLog(`${tr('Survey send failed', '问卷发送失败', 'Umfrage konnte nicht gesendet werden')}: ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(`${tr('Loading report data', '加载报告数据', 'Lade Berichtsdaten')}: ${props.reportId}`)
    ...
    addLog(`${tr('Failed to load report', '加载报告失败', 'Bericht konnte nicht geladen werden')}: ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(tr('Report data loaded', '报告数据已加载', 'Berichtsdaten geladen'))
    }
  } catch (err) {
    addLog(`${tr('Failed to load report logs', '加载报告日志失败', 'Berichtsprotokolle konnten nicht geladen werden')}: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${tr('Loaded', '已加载', 'Geladen')} ${profiles.value.length} ${tr('simulated individuals', '个模拟个体', 'simulierte Individuen')}`)
    }
  } catch (err) {
    addLog(`${tr('Failed to load simulated individuals', '加载模拟个体失败', 'Simulierte Individuen konnten nicht geladen werden')}: ${err.message}`)
  }
}
...
// Lifecycle
onMounted(() => {
  addLog(tr('Step 5 Deep Interaction initializing', '步骤 5 深度交互初始化中', 'Schritt 5 Tiefe Interaktion wird initialisiert'))
  loadReportData()
  loadProfiles()
