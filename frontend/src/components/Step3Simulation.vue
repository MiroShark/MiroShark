  ...
  <div class="simulation-panel">
    ...
    <div class="actions-bar">
      ...
      <button
        ...
        @click="emit('go-back')"
      >{{ $tr('← Config', '← 配置', '← Konfiguration') }}</button>
      ...
      >
        ...
        {{ isStopping ? $tr('Pausing...', '暂停中…', 'Pausierung…') : $tr('Pause', '暂停', 'Pause') }}
      </button>
      ...
      <button
        ...
        @click="handleRestart"
      >
        ↻ {{ runStatus.runner_status === 'failed' ? $tr('Restart (failed)', '重启(失败)', 'Neustart (fehlgeschlagen)') : $tr('Restart', '重启', 'Neustart') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Generate a publishable article brief from simulation results', '从模拟结果生成可发布的文章简报', 'Aus Simulationsergebnissen einen veröffentlichbaren Artikelüberblick generieren')"
      >
        ▤ {{ $tr('Article', '文章', 'Artikel') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Agent influence leaderboard', '智能体影响力排行榜', 'Agent-Einfluss-Rangliste')"
      >
        ◈ {{ $tr('Influence', '影响力', 'Einfluss') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Aggregate belief drift chart', '群体信念漂移图', 'Aggregiertes Überzeugungsdrift-Diagramm')"
      >
        ◎ {{ $tr('Drift', '漂移', 'Drift') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Agent interaction network graph', '智能体互动网络图', 'Agent-Interaktionsnetzwerk-Diagramm')"
      >
        ⬡ {{ $tr('Network', '网络', 'Netzwerk') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Agent demographic breakdown (age, gender, country, actor type, platform)', '智能体人口统计分布(年龄、性别、国家、角色类型、平台)', 'Agent-Demografieaufschlüsselung (Alter, Geschlecht, Land, Akteurtyp, Plattform)')"
      >
        ◇ {{ $tr('Demographics', '人口统计', 'Demografie') }}
      </button>
      ...
      <button
        ...
        :title="$tr('Live prediction market price chart', '实时预测市场价格图', 'Live-Vorhersagemarkt-Preisdiagramm')"
      >
        <img src="/pm.png" class="btn-platform-icon" alt="" />
        {{ $tr('Markets', '市场', 'Märkte') }}
      </button>
      ...
      <button
        ...
        :title="$tr('What If? — remove agents and recompute belief drift from existing trajectory', '假如?— 移除智能体并基于现有轨迹重新计算信念漂移', 'Was wäre wenn? — Agenten entfernen und Überzeugungsdrift aus bestehendem Verlauf neu berechnen')"
      >
        ◐ {{ $tr('What If?', '假如?', 'Was wäre wenn?') }}
      </button>
      ...
      <button
        ...
        :title="directorEventsTotal >= 10 ? $tr('Director Mode — max events reached', '导演模式 — 已达事件上限', 'Regisseur-Modus — maximale Ereignisanzahl erreicht') : $tr('Director Mode — inject a breaking event into the simulation', '导演模式 — 向模拟中注入突发事件', 'Regisseur-Modus — Breaking-Ereignis in die Simulation injizieren')"
      >
        ⚡ {{ $tr('Director', '导演', 'Regisseur') }}
      ...
      <button
        ...
        :title="$tr('Fork this simulation with a narrative injection scheduled for a specific round', '在指定轮次插入叙事注入,以分支此模拟', 'Diese Simulation mit einer Narrations-Injektion für eine bestimmte Runde forken')"
      >
        ⤷ {{ $tr('Branch', '分支', 'Branch') }}
      </button>
      ...
      >
        ...
        {{ isStarting ? $tr('Resuming...', '继续中…', 'Fortsetzen…') : $tr('Resume', '继续', 'Fortsetzen') }}
      </button>
      ...
      >
        ...
        <template v-if="isGeneratingReport">{{ $tr('Starting...', '启动中…', 'Starten…') }}</template>
        <template v-else-if="phase === 1">{{ $tr('Skip to Report ⟶', '跳至报告 ⟶', 'Zum Bericht springen ⟶') }}</template>
        <template v-else>{{ $tr('Report →', '报告 →', 'Bericht →') }}</template>
      </button>
    </div>

    <!-- Total Events Summary -->
    <div class="events-summary">
      <span class="events-label">{{ $tr('TOTAL EVENTS:', '总事件数:', 'GESAMT-EREIGNISSE:') }}</span>
    ...
    <div v-if="showQualityPanel && qualityData" class="quality-panel">
      <div class="qp-header">
        <span class="qp-title">{{ $tr('QUALITY DIAGNOSTICS', '质量诊断', 'QUALITÄTSDIAGNOSE') }}</span>
        <button class="qp-close" @click="showQualityPanel = false">×</button>
      </div>
      <div class="qp-metrics">
        <div class="qp-metric">
          <span class="qp-label">{{ $tr('Participation', '参与度', 'Beteiligung') }}</span>
        ...
        <div v-if="qualityData.stance_entropy !== null" class="qp-metric">
          <span class="qp-label">{{ $tr('Stance Diversity', '立场多样性', 'Haltungsvielfalt') }}</span>
        ...
        <div class="qp-metric">
          <span class="qp-label">{{ $tr('Cross-Platform', '跨平台', 'Plattformübergreifend') }}</span>
        ...
        <div v-if="qualityData.convergence_round !== null" class="qp-metric">
          <span class="qp-label">{{ $tr('Consensus', '共识', 'Konsens') }}</span>
          <span class="qp-val qp-convergence">{{ $tr('Round', '第', 'Runde') }} {{ qualityData.convergence_round }} {{ $tr('', '轮', '') }}</span>
        </div>
      </div>
      <div v-if="qualityData.suggestions && qualityData.suggestions.length" class="qp-suggestions">
        <div class="qp-suggestions-title">{{ $tr('Try for next run:', '下次运行可尝试:', 'Beim nächsten Durchlauf ausprobieren:') }}</div>
      ...
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed, selected: filteredPlatform === 'twitter', dimmed: filteredPlatform && filteredPlatform !== 'twitter' }" @click="filterByPlatform('twitter')">
          <div class="platform-left">
            ...
            <span v-if="runStatus.twitter_completed" class="status-badge done">{{ $tr('done', '完成', 'Fertig') }}</span>
          </div>
          <div class="platform-stats">
            <span class="stat"><span class="stat-label">{{ $tr('RND', '轮次', 'RND') }}</span><span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span></span>
            <span class="stat"><span class="stat-label">{{ $tr('TIME', '时间', 'ZEIT') }}</span><span class="stat-value mono">{{ twitterElapsedTime }}</span></span>
            <span class="stat"><span class="stat-label">{{ $tr('ACTS', '动作', 'AKTIONEN') }}</span><span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span></span>
          </div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed, selected: filteredPlatform === 'reddit', dimmed: filteredPlatform && filteredPlatform !== 'reddit' }" @click="filterByPlatform('reddit')">
          <div class="platform-left">
            ...
            <span v-if="runStatus.reddit_completed" class="status-badge done">{{ $tr('done', '完成', 'Fertig') }}</span>
          </div>
          <div class="platform-stats">
            <span class="stat"><span class="stat-label">{{ $tr('RND', '轮次', 'RND') }}</span><span class="stat-value mono">{{ runStatus.reddit_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span></span>
            <span class="stat"><span class="stat-label">{{ $tr('TIME', '时间', 'ZEIT') }}</span><span class="stat-value mono">{{ redditElapsedTime }}</span></span>
            <span class="stat"><span class="stat-label">{{ $tr('ACTS', '动作', 'AKTIONEN') }}</span><span class="stat-value mono">{{ runStatus.reddit_actions_count || 0 }}</span></span>
          </div>
        ...
        <div class="platform-status polymarket" :class="{ active: runStatus.polymarket_running, completed: runStatus.polymarket_completed, selected: filteredPlatform === 'polymarket', dimmed: filteredPlatform && filteredPlatform !== 'polymarket' }" @click="filterByPlatform('polymarket')">
          <div class="platform-left">
            ...
            <span v-if="runStatus.polymarket_completed" class="status-badge done">{{ $tr('done', '完成', 'Fertig') }}</span>
          </div>
          <div class="platform-stats">
            <span class="stat"><span class="stat-label">{{ $tr('RND', '轮次', 'RND') }}</span><span class="stat-value mono">{{ runStatus.polymarket_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span></span>
            <span class="stat"><span class="stat-label">{{ $tr('TIME', '时间', 'ZEIT') }}</span><span class="stat-value mono">{{ polymarketElapsedTime }}</span></span>
            <span class="stat"><span class="stat-label">{{ $tr('TRADES', '交易', 'TRADES') }}</span><span class="stat-value mono">{{ runStatus.polymarket_actions_count || 0 }}</span></span>
          </div>
    ...
    <div v-if="showDirector" class="influence-overlay director-panel">
      <div class="director-header">
        <div class="director-title">
          ...
          <span class="director-label">{{ $tr('DIRECTOR MODE', '导演模式', 'REGISSEUR-MODUS') }}</span>
        </div>
        <span class="director-hint">{{ $tr('Inject a breaking event — all agents receive it at the next round boundary', '注入一个突发事件 — 所有智能体将在下一轮边界接收到它', 'Ein Breaking-Ereignis injizieren — alle Agenten erhalten es an der nächsten Rundengrenze') }}</span>
      </div>
      ...
      <div class="director-form">
        <textarea
          ...
          :placeholder="$tr(`Describe the event (e.g. 'Central bank unexpectedly raised rates by 100bps')...`, `描述事件(例如「央行意外加息 100 个基点」)…`, `Beschreibe das Ereignis (z. B. „Zentralbank erhöhte Zinsen unerwartet um 100 Bp.“…`)"
          maxlength="500"
        ...
        <div class="director-form-footer">
          ...
          >
            ...
            {{ isInjectingEvent ? $tr('Injecting...', '注入中…', 'Injizieren…') : directorEventsTotal >= 10 ? $tr('Max events reached', '已达事件上限', 'Maximale Ereignisanzahl erreicht') : $tr('Inject Event', '注入事件', 'Ereignis injizieren') }}
          </button>
        </div>
      ...
      <div v-if="directorEventHistory.length > 0" class="director-history">
        <div class="director-history-title">{{ $tr('Injected Events', '已注入事件', 'Injizierte Ereignisse') }}</div>
        <button
          v-for="evt in directorEventHistory"
          :key="evt.id"
          class="director-event-card director-event-card-clickable"
          type="button"
          :title="$tr('Show this event on the timeline', '在时间线上显示此事件', 'Dieses Ereignis in der Zeitleiste anzeigen')"
          @click="jumpToDirectorEvent(evt)"
        >
          <div class="director-event-header">
            <span class="director-event-badge">⚡ {{ $tr('ROUND', '轮次', 'RUNDE') }} {{ evt.injected_at_round || evt.submitted_at_round }}</span>
      ...
      <div v-if="directorPendingEvents.length > 0" class="director-history">
        <div class="director-history-title">{{ $tr('Pending (will inject next round)', '等待中(将在下一轮注入)', 'Ausstehend (wird nächste Runde injiziert)') }}</div>
        <div
        ...
        >
          <div class="director-event-header">
            <span class="director-event-badge pending-badge">◌ {{ $tr('QUEUED', '已排队', 'IN WARTESCHLANGE') }}</span>
          </div>
          <div class="director-event-text">{{ evt.event_text }}</div>
        </div>
      </div>
    ...
    <div v-show="!showInfluence && !showBeliefDrift && !showDirector && !showNetwork && !showDemographics && !showBranch && !showWhatIf && !showPolymarketChart" class="main-content-area" ref="scrollContainer" @scroll="onTimelineScroll">
      ...
      <div v-if="filteredPlatform" class="agent-filter-bar">
        <div class="filter-info">
          ...
          <span class="filter-count">{{ chronologicalActions.length }} {{ $tr('events', '事件', 'Ereignisse') }}</span>
        </div>
        <button class="filter-clear" @click="clearPlatformFilter">{{ $tr('Clear', '清除', 'Löschen') }}</button>
      </div>
      ...
      <div v-if="filteredAgent" class="agent-filter-bar">
        ...
        <button class="filter-clear" @click="clearAgentFilter">{{ $tr('Clear', '清除', 'Löschen') }}</button>
      </div>
      ...
      <div class="timeline-feed">
          ...
          >
            ...
            <div v-if="action._isDirectorEvent" class="timeline-card director-card">
              <div class="director-inline-banner">
                ...
                <span class="director-inline-label">{{ $tr('BREAKING — Round', '突发 — 第', 'BREAKING — Runde') }} {{ action.round_num }}{{ $tr('', ' 轮', '') }}</span>
              </div>
            ...
            <div v-else class="timeline-card">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || $tr('User', '用户', 'Nutzer') }}</span>
                    </div>
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $tr('Reposted from @', '转发自 @', 'Geteilt von @') }}{{ action.action_args?.original_author_name || $tr('User', '用户', 'Nutzer') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $tr('Liked @', '点赞了 @', 'Gefällt @') }}{{ action.action_args?.post_author_name || $tr('User', '用户', 'Nutzer') }}{{ $tr(`'s post`, ' 的帖子', ' Beitrag') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $tr('Reply to post #', '回复帖子 #', 'Antwort auf Beitrag #') }}{{ action.action_args.post_id }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $tr('Search Query:', '搜索查询:', 'Suchanfrage:') }}</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $tr('Followed @', '关注了 @', 'Folgt @') }}{{ action.action_args?.target_user_name || action.action_args?.target_user || action.action_args?.user_id || $tr('User', '用户', 'Nutzer') }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'DISLIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $tr('Disliked @', '踩了 @', 'Missfällt @') }}{{ action.action_args?.post_author_name || $tr('User', '用户', 'Nutzer') }}{{ $tr(`'s post`, ' 的帖子', ' Beitrag') }}</span>
                  </div>
                  <div v-if="action.action_args?.post_content" class="liked-content">
                    "{{ truncateContent(action.action_args.post_content, 120) }}"
                  </div>
                ...
                <template v-if="action.action_type === 'DISLIKE_COMMENT'">
                  <div class="like-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                    <span class="like-label">{{ $tr('Disliked @', '踩了 @', 'Missfällt @') }}{{ action.action_args?.comment_author_name || $tr('User', '用户', 'Nutzer') }}{{ $tr(`'s comment`, ' 的评论', ' Kommentar') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'MUTE'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $tr('Muted @', '静音了 @', 'Stummgeschaltet @') }}{{ action.action_args?.target_user_name || action.action_args?.user_id || $tr('User', '用户', 'Nutzer') }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $tr('Upvoted', '顶', 'Hochgestimmt') : $tr('Downvoted', '踩', 'Runtergestimmt') }} {{ $tr('Post', '帖子', 'Beitrag') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $tr('Action Skipped', '跳过此动作', 'Aktion übersprungen') }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'BUY_SHARES'">
                  <div class="trade-info">
                    ...
                    <span class="trade-detail">{{ formatShares(action.action_args?.shares) }} <strong>{{ action.action_args?.outcome }}</strong> {{ $tr('shares', '份额', 'Anteile') }}</span>
                  ...
                  <div v-if="action.action_args?.market_id" class="market-ref">{{ $tr('Market #', '市场 #', 'Markt #') }}{{ action.action_args.market_id }}</div>
                </template>
                ...
                <template v-if="action.action_type === 'BROWSE_MARKETS'">
                  <div class="idle-info"><span class="idle-label">{{ $tr('Browsed active markets', '浏览活跃市场', 'Aktive Märkte durchsucht') }}</span></div>
                </template>
                <template v-if="action.action_type === 'VIEW_PORTFOLIO'">
                  <div class="idle-info"><span class="idle-label">{{ $tr('Checked portfolio', '查看持仓', 'Portfolio geprüft') }}</span></div>
                </template>
              ...
              <div class="card-footer">
                <span class="time-tag">{{ $tr('R', '第', 'R') }}{{ action.round_num }}{{ $tr('', ' 轮', '') }} • {{ formatActionTime(action.timestamp) }}</span>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>{{ $tr('Waiting for agent actions...', '等待智能体动作…', 'Warte auf Agent-Aktionen…') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="system-logs" :class="{ collapsed: monitorCollapsed }">
      <div class="log-header" @click="monitorCollapsed = !monitorCollapsed">
        <span class="log-title">{{ $tr('SIMULATION MONITOR', '模拟监控', 'SIMULATIONS-MONITOR') }} <span class="log-toggle">{{ monitorCollapsed ? '▲' : '▼' }}</span></span>
        <span class="log-id copyable" @click.stop="copySimId" :title="copied ? $tr('Copied!', '已复制!', 'Kopiert!') : $tr('Click to copy', '点击复制', 'Zum Kopieren klicken')">{{ simulationId || $tr('NO_SIMULATION', '无模拟', 'KEINE_SIMULATION') }}{{ copied ? ' ✓' : '' }}</span>
      </div>
      ...
      <div v-if="showArticleDrawer" class="article-drawer-overlay" @click.self="showArticleDrawer = false">
        <div class="article-drawer">
          <div class="article-drawer-header">
            <span class="article-drawer-title">{{ $tr('Generated Article', '已生成的文章', 'Generierter Artikel') }}</span>
            <div class="article-drawer-actions">
              <button
                class="article-action-btn"
                :disabled="isGeneratingArticle || !articleText"
                @click="copyArticle"
                :title="articleCopied ? $tr('Copied!', '已复制!', 'Kopiert!') : $tr('Copy to clipboard', '复制到剪贴板', 'In Zwischenablage kopieren')"
              >{{ articleCopied ? $tr('Copied!', '已复制!', 'Kopiert!') : $tr('Copy', '复制', 'Kopieren') }}</button>
              <button
                class="article-action-btn"
                :disabled="isGeneratingArticle || !articleText"
                @click="downloadArticle"
                :title="$tr('Download as .md', '下载为 .md 文件', 'Als .md herunterladen')"
              >{{ $tr('Download .md', '下载 .md', '.md herunterladen') }}</button>
          ...
          <div class="article-drawer-body">
            ...
            <div v-if="isGeneratingArticle" class="article-loading">
              ...
              <span class="article-loading-label">{{ $tr('Generating article from simulation data...', '正在从模拟数据生成文章…', 'Artikel aus Simulationsdaten wird generiert…') }}</span>
            </div>

            <!-- Error state -->
            <div v-else-if="articleError" class="article-error">
              <span class="article-error-msg">{{ articleError }}</span>
              <button class="article-action-btn" @click="generateArticle">{{ $tr('Retry', '重试', 'Erneut versuchen') }}</button>
            </div>
...
const qualityTooltip = computed(() => {
  ...
  if (!q) return ''
  const parts = [tr(`Health: ${q.health}`, `健康度:${q.health}`, `Gesundheit: ${q.health}`), tr(`Participation ${Math.round(q.participation_rate * 100)}%`, `参与度 ${Math.round(q.participation_rate * 100)}%`, `Beteiligung ${Math.round(q.participation_rate * 100)}%`)]
  if (q.stance_entropy !== null) {
    ...
    parts.push(tr(`Diversity: ${level}`, `多样性:${level}`, `Vielfalt: ${level}`))
  }
  return parts.join(' · ')
})
...
const handleInjectEvent = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      directorError.value = res.error || tr('Failed to inject event', '注入事件失败', 'Ereignis konnte nicht injiziert werden')
    }
  } catch (err) {
    directorError.value = err.response?.data?.error || err.message || tr('Failed to inject event', '注入事件失败', 'Ereignis konnte nicht injiziert werden')
  } finally {
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog(tr('Starting dual-platform parallel simulation...', '正在启动双平台并行模拟…', 'Dual-Plattform-Parallelsimulation wird gestartet…'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(tr(`Set max simulation rounds: ${props.maxRounds}`, `设定最大模拟轮次:${props.maxRounds}`, `Maximale Simulationsrunden festgelegt: ${props.maxRounds}`))
    }

    addLog(tr('Dynamic graph memory update mode enabled', '已启用动态图记忆更新模式', 'Dynamischer Graphspeicher-Aktualisierungsmodus aktiviert'))

    const res = await startSimulation(params)

    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(tr('Old simulation logs cleaned, restarting simulation', '已清理旧的模拟日志,重新启动模拟', 'Alte Simulations-Logs bereinigt, Simulation wird neu gestartet'))
      }
      addLog(tr('Simulation engine started successfully', '模拟引擎启动成功', 'Simulationsmotor erfolgreich gestartet'))
      addLog(tr(`  ├─ PID: ${res.data.process_pid || '-'}`, `  ├─ PID:${res.data.process_pid || '-'}`, `  ├─ PID: ${res.data.process_pid || '-'}`))
      ...
      startError.value = res.error || tr('Start failed', '启动失败', 'Start fehlgeschlagen')
      addLog(tr(`Start failed: ${res.error || 'Unknown error'}`, `启动失败:${res.error || '未知错误'}`, `Start fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(tr(`Start error: ${err.message}`, `启动出错:${err.message}`, `Startfehler: ${err.message}`))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleResume = async () => {
  ...
  addLog(tr(`Resuming simulation from round ${fromRound}...`, `从第 ${fromRound} 轮继续模拟…`, `Simulation wird ab Runde ${fromRound} fortgesetzt…`))
  ...
  try {
    ...
    if (res.success && res.data) {
      addLog(tr(`Resumed from round ${res.data.resumed_from_round || fromRound}`, `已从第 ${res.data.resumed_from_round || fromRound} 轮继续`, `Ab Runde ${res.data.resumed_from_round || fromRound} fortgesetzt`))
      addLog(tr(`  ├─ PID: ${res.data.process_pid || '-'}`, `  ├─ PID:${res.data.process_pid || '-'}`, `  ├─ PID: ${res.data.process_pid || '-'}`))
      phase.value = 1
      ...
      startError.value = res.error || tr('Resume failed', '继续失败', 'Fortsetzen fehlgeschlagen')
      addLog(tr(`Resume failed: ${res.error || 'Unknown error'}`, `继续失败:${res.error || '未知错误'}`, `Fortsetzen fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(tr(`Resume error: ${err.message}`, `继续出错:${err.message}`, `Fehler beim Fortsetzen: ${err.message}`))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleRestart = async () => {
  if (!props.simulationId) return
  addLog(tr('Restarting simulation from scratch...', '从零重新启动模拟…', 'Simulation wird von Grund auf neu gestartet…'))
  resetAllState()
  doStartSimulation()
}
...
const handleStopSimulation = async () => {
  ...
  addLog(tr('Stopping simulation...', '正在停止模拟…', 'Simulation wird gestoppt…'))

  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })

    if (res.success) {
      addLog(tr('Simulation stopped', '模拟已停止', 'Simulation gestoppt'))
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(tr(`Stop failed: ${res.error || 'Unknown error'}`, `停止失败:${res.error || '未知错误'}`, `Stopp fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}`))
    }
  } catch (err) {
    addLog(tr(`Stop error: ${err.message}`, `停止出错:${err.message}`, `Stoppfehler: ${err.message}`))
  } finally {
    isStopping.value = false
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(tr('All platform simulations have ended', '所有平台模拟均已结束', 'Alle Plattformsimulationen beendet'))
        }
        addLog(tr('Simulation completed', '模拟已完成', 'Simulation abgeschlossen'))
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(tr('Report generation request already sent, please wait...', '报告生成请求已发送,请稍候…', 'Berichterstellungsanfrage bereits gesendet, bitte warten…'))
    return
  }
  ...
  if (phase.value === 1) {
    addLog(tr('Stopping simulation before generating report...', '正在停止模拟以生成报告…', 'Simulation wird vor der Berichterstellung gestoppt…'))
    try {
      await stopSimulation({ simulation_id: props.simulationId })
      phase.value = 2
      stopPolling()
      addLog(tr('Simulation stopped — proceeding with partial data', '模拟已停止 — 将使用部分数据继续', 'Simulation gestoppt — wird mit Teildaten fortgefahren'))
      emit('update-status', 'completed')
    } catch (err) {
      addLog(tr(`Warning: could not stop simulation (${err.message}), proceeding anyway`, `警告:无法停止模拟(${err.message}),仍将继续`, `Warnung: Simulation konnte nicht gestoppt werden (${err.message}), wird trotzdem fortgefahren`))
      stopPolling()
      phase.value = 2
    }
  }
  ...
  try {
    ...
    addLog(tr('Checking for existing report...', '正在检查已有报告…', 'Vorhandener Bericht wird geprüft…'))
    ...
    if (res.success && res.data) {
      ...
      if (res.data.already_generated) {
        addLog(tr(`Found existing report: ${reportId}`, `找到已有报告:${reportId}`, `Vorhandener Bericht gefunden: ${reportId}`))
      } else {
        addLog(tr(`Report generation started: ${reportId}`, `报告生成已启动:${reportId}`, `Berichterstellung gestartet: ${reportId}`))
      }
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(tr(`Failed to start report generation: ${res.error || 'Unknown error'}`, `启动报告生成失败:${res.error || '未知错误'}`, `Berichterstellung konnte nicht gestartet werden: ${res.error || 'Unbekannter Fehler'}`))
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(tr(`Report generation error: ${err.message}`, `报告生成出错:${err.message}`, `Fehler bei der Berichterstellung: ${err.message}`))
    isGeneratingReport.value = false
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (status === 'running' || status === 'starting') {
        ...
        addLog(tr(`Reconnecting to running simulation (round ${res.data.current_round}/${res.data.total_rounds})...`, `正在重新连接运行中的模拟(第 ${res.data.current_round}/${res.data.total_rounds} 轮)…`, `Verbindung zur laufenden Simulation wird wiederhergestellt (Runde ${res.data.current_round}/${res.data.total_rounds})…`))
        runStatus.value = res.data
        phase.value = 1
        emit('update-status', 'processing')
        startStatusPolling()
      ...
      if (status === 'completed' || status === 'stopped') {
        ...
        addLog(tr(`Previous simulation found: ${status} (${totalActions} actions, round ${res.data.current_round}/${res.data.total_rounds})`, `发现之前的模拟:${status}(${totalActions} 个动作,第 ${res.data.current_round}/${res.data.total_rounds} 轮)`, `Vorherige Simulation gefunden: ${status} (${totalActions} Aktionen, Runde ${res.data.current_round}/${res.data.total_rounds})`))
        runStatus.value = res.data
        phase.value = 2
        emit('update-status', 'completed')
      ...
      if (status === 'failed') {
        ...
        const totalActions = (res.data.twitter_actions_count || 0) + (res.data.reddit_actions_count || 0)
        if (totalActions > 0) {
          addLog(tr(`Previous simulation crashed at round ${res.data.current_round}/${res.data.total_rounds} with ${totalActions} actions`, `之前的模拟在第 ${res.data.current_round}/${res.data.total_rounds} 轮崩溃,共 ${totalActions} 个动作`, `Vorherige Simulation bei Runde ${res.data.current_round}/${res.data.total_rounds} mit ${totalActions} Aktionen abgestürzt`))
          addLog(tr('You can generate a report from partial data or restart', '您可以基于部分数据生成报告或重新启动', 'Du kannst einen Bericht aus Teildaten generieren oder neu starten'))
          runStatus.value = res.data
        ...
        addLog(tr('Previous simulation failed with no data — starting fresh', '之前的模拟失败且无数据 — 重新开始', 'Vorherige Simulation ohne Daten fehlgeschlagen — wird neu gestartet'))
      }
    }
  } catch (err) {
...
const generateArticle = async () => {
  ...
  try {
    ...
    if (res.success && res.data?.article_text) {
      ...
      articleError.value = res.error || tr('Failed to generate article.', '生成文章失败。', 'Artikel konnte nicht generiert werden.')
    }
  } catch (err) {
    articleError.value = err?.message || tr('Network error generating article.', '生成文章时发生网络错误。', 'Netzwerkfehler beim Generieren des Artikels.')
  } finally {
...
onMounted(() => {
  addLog(tr('Step3 Simulation Run initialized', '第 3 步 模拟运行已初始化', 'Schritt 3 Simulationslauf initialisiert'))
  tryResumeOrStart()
})
