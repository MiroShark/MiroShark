...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(tr(`Starting simulation, custom rounds: ${customMaxRounds.value} rounds`, `开始模拟,自定义轮次:${customMaxRounds.value} 轮`, { de: `Simulation wird gestartet, benutzerdefinierte Runden: ${customMaxRounds.value} Runden` }))
  } else {
    // User chose to keep auto-generated rounds, no max_rounds parameter passed
    addLog(tr(`Starting simulation, using auto-configured rounds: ${autoGeneratedRounds.value} rounds`, `开始模拟,使用自动配置轮次:${autoGeneratedRounds.value} 轮`, { de: `Simulation wird gestartet, automatisch konfigurierte Runden: ${autoGeneratedRounds.value} Runden` }))
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(tr(`Read ${res.data.expected_entities_count} entities from knowledge graph`, `从知识图谱读取 ${res.data.expected_entities_count} 个实体`, { de: `${res.data.expected_entities_count} Entitäten aus dem Wissensgraphen gelesen` }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(tr(`  └─ Entity types: ${res.data.entity_types.join(', ')}`, `  └─ 实体类型:${res.data.entity_types.join('、')}`, { de: `  └─ Entitätstypen: ${res.data.entity_types.join(', ')}` }))
        }
      }
      ...
      addLog(tr(`Preparation failed: ${res.error || 'Unknown error'}`, `准备失败:${res.error || '未知错误'}`, { de: `Vorbereitung fehlgeschlagen: ${res.error || 'Unbekannter Fehler'}` }))
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(tr(`Preparation error: ${err.message}`, `准备出错:${err.message}`, { de: `Vorbereitungsfehler: ${err.message}` }))
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(tr(`✗ Preparation failed: ${data.error || 'Unknown error'}`, `✗ 准备失败:${data.error || '未知错误'}`, { de: `✗ Vorbereitung fehlgeschlagen: ${data.error || 'Unbekannter Fehler'}` }))
        stopPolling()
        stopProfilesPolling()
      }
    }
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(tr(`✓ All ${currentCount} agent personas generated`, `✓ 已生成全部 ${currentCount} 个智能体人设`, { de: `✓ Alle ${currentCount} Agent-Personas generiert` }))
        }
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          addLog(tr(`  ├─ Agent count: ${data.summary.total_agents}`, `  ├─ 智能体数量:${data.summary.total_agents}`, { de: `  ├─ Agentenanzahl: ${data.summary.total_agents}` }))
          addLog(tr(`  ├─ Simulation duration: ${data.summary.simulation_hours} hours`, `  ├─ 模拟时长:${data.summary.simulation_hours} 小时`, { de: `  ├─ Simulationsdauer: ${data.summary.simulation_hours} Stunden` }))
          addLog(tr(`  ├─ Initial posts: ${data.summary.initial_posts_count}`, `  ├─ 初始帖子:${data.summary.initial_posts_count}`, { de: `  ├─ Initiale Beiträge: ${data.summary.initial_posts_count}` }))
          addLog(tr(`  ├─ Hot topics: ${data.summary.hot_topics_count}`, `  ├─ 热门话题:${data.summary.hot_topics_count}`, { de: `  ├─ Trendthemen: ${data.summary.hot_topics_count}` }))
          addLog(tr(`  └─ Platform config: Twitter ${data.summary.has_twitter_config ? '✓' : '✗'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`, `  └─ 平台配置:Twitter ${data.summary.has_twitter_config ? '✓' : '✗'},Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`, { de: `  └─ Plattformkonfiguration: Twitter ${data.summary.has_twitter_config ? '✓' : '✗'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}` }))
        }

        // Show time configuration details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(tr(`Time config: ${tc.minutes_per_round} minutes per round, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds total`, `时间配置:每轮 ${tc.minutes_per_round} 分钟,共 ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} 轮`, { de: `Zeitkonfiguration: ${tc.minutes_per_round} Minuten pro Runde, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} Runden gesamt` }))
        }

        // Show event configuration
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(tr(`Narrative direction: ${narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative}`, `叙事方向:${narrative.length > 50 ? narrative.substring(0, 50) + '…' : narrative}`, { de: `Narrativrichtung: ${narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative}` }))
        }
...
const loadPreparedData = async () => {
  ...
  addLog(tr(`Loaded ${profiles.value.length} agent personas`, `已加载 ${profiles.value.length} 个智能体人设`, { de: `${profiles.value.length} Agent-Personas geladen` }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Show detailed config summary
        if (res.data.summary) {
          addLog(tr(`  ├─ Agent count: ${res.data.summary.total_agents}`, `  ├─ 智能体数量:${res.data.summary.total_agents}`, { de: `  ├─ Agentenanzahl: ${res.data.summary.total_agents}` }))
          addLog(tr(`  ├─ Simulation duration: ${res.data.summary.simulation_hours} hours`, `  ├─ 模拟时长:${res.data.summary.simulation_hours} 小时`, { de: `  ├─ Simulationsdauer: ${res.data.summary.simulation_hours} Stunden` }))
    ...
    addLog(tr(`Failed to load configuration: ${err.message}`, `加载配置失败:${err.message}`, { de: `Konfiguration konnte nicht geladen werden: ${err.message}` }))
    emit('update-status', 'error')
  }
}
