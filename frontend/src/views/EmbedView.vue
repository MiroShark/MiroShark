    ...
    <template v-else>
      <header v-if="!chartOnly" class="embed-header">
        ...
        <div class="embed-meta">
          ...
          <span v-if="hasRounds" class="embed-pill">
            {{ $tr('Round', '轮次', 'Runde') }} {{ summary.current_round }}/{{ summary.total_rounds || summary.current_round }}
          </span>
          <span class="embed-pill">{{ summary.profiles_count || 0 }} {{ $tr('agents', '智能体', 'Agenten') }}</span>
          <span
            v-if="costLabel"
            class="embed-pill cost"
            :title="$tr('Estimated cost to run this simulation (lower bound, from logged LLM calls)', '运行此模拟的预估成本(基于已记录的 LLM 调用,为下限)', 'Geschätzte Kosten für diese Simulation (Untergrenze, aus protokollierten LLM-Aufrufen)')"
          >
            {{ costLabel }}
          </span>
      ...
      <div class="embed-chart-wrap">
        ...
        <div v-else class="embed-empty-chart">
          <span>{{ $tr('No belief trajectory yet', '暂无信念轨迹', 'Noch keine Überzeugungstrajektorie') }}</span>
        </div>

        <div v-if="hasBelief && !chartOnly" class="embed-final-row">
          <span class="final-chip bullish">
            <span class="chip-dot"></span>
            {{ $tr('Bullish', '看涨', 'Optimistisch') }} {{ finalBullish }}%
          </span>
          <span class="final-chip neutral">
            <span class="chip-dot"></span>
            {{ $tr('Neutral', '中立', 'Neutral') }} {{ finalNeutral }}%
          </span>
          <span class="final-chip bearish">
            <span class="chip-dot"></span>
            {{ $tr('Bearish', '看跌', 'Pessimistisch') }} {{ finalBearish }}%
          </span>
        </div>
      </div>
      ...
      <footer v-if="!chartOnly" class="embed-footer">
        ...
        <a class="embed-footer-link" :href="simulationUrl" target="_blank" rel="noopener">
          {{ $tr('Powered by', '技术支持:', 'Bereitgestellt von') }} <strong>MiroShark</strong> ↗
        </a>
      </footer>
    </template>
  </div>
...
const scenarioTitle = computed(() => {
  ...
  if (!raw) return tr('Untitled simulation', '未命名模拟', 'Unbenannte Simulation')
...
const statusLabel = computed(() => {
  if (!summary.value) return tr('Unknown', '未知', 'Unbekannt')
  const s = (summary.value.runner_status || summary.value.status || '').toLowerCase()
  if (s === 'completed' || s === 'finished' || s === 'stopped') return tr('Completed', '已完成', 'Abgeschlossen')
  if (s === 'running' || s === 'in_progress') return tr('Running', '运行中', 'Läuft')
  if (s === 'error' || s === 'failed') return tr('Failed', '失败', 'Fehlgeschlagen')
  return s ? s.charAt(0).toUpperCase() + s.slice(1) : tr('Ready', '就绪', 'Bereit')
})
...
const consensusLabel = computed(() => {
  ...
  return `${tr('Consensus formed at round', '共识形成于第', 'Konsens erreicht in Runde')} ${b.consensus_round}${tr('', '轮', '')} (${b.consensus_stance})`
})
...
const resolutionLabel = computed(() => {
  ...
  if (r.accuracy_score !== null && r.accuracy_score !== undefined) {
    if (r.accuracy_score >= 1.0) return `✓ ${tr('Correct', '正确', 'Richtig')} · ${tr('Actual', '实际', 'Ergebnis')} ${r.actual_outcome}`
    if (r.accuracy_score <= 0.0) return `✗ ${tr('Missed', '未中', 'Verfehlt')} · ${tr('Actual', '实际', 'Ergebnis')} ${r.actual_outcome}`
    return `~ ${tr('Split', '部分', 'Geteilt')} · ${tr('Actual', '实际', 'Ergebnis')} ${r.actual_outcome}`
  }
  return `${tr('Actual', '实际', 'Ergebnis')} ${r.actual_outcome}`
})
...
const chartAriaLabel = computed(() => {
  if (!hasBelief.value) return tr('No belief trajectory', '无信念轨迹', 'Keine Überzeugungstrajektorie')
  return `${tr('Belief drift across', '信念漂移历经', 'Überzeugungsdrift über')} ${summary.value.belief.rounds.length} ${tr('rounds', '轮次', 'Runden')}`
})
...
const fetchData = async () => {
  if (!simulationId.value) {
    error.value = tr('Missing simulation id', '缺少模拟 ID', 'Simulations-ID fehlt')
    loading.value = false
    return
  }
  try {
    ...
    if (res?.success) {
      ...
      error.value = res?.error || tr('Failed to load simulation', '加载模拟失败', 'Simulation konnte nicht geladen werden')
    }
  } catch (err) {
    error.value = err?.response?.data?.error || err?.message || tr('Failed to load simulation', '加载模拟失败', 'Simulation konnte nicht geladen werden')
  } finally {
    loading.value = false
  }
